...
public class SanPhamDAO {
	...
	public static String getMaLoaiByMaSP(String maSP) {
	    String maLoai = null;

	    try (Connection con = ConnectDB.getConection();
	         PreparedStatement statement = con.prepareStatement("SELECT maLoaiSP FROM SanPham WHERE maSP = ?")) {

	        statement.setString(1, maSP);
	        ResultSet rs = statement.executeQuery();

	        if (rs.next()) {
	            maLoai = rs.getString("maLoaiSP");
	        }
	...
	public static List<SanPham> getDSSanPhamByTenLoai(String tenLoai) throws Exception {
	    List<SanPham> danhSachSanPham = new ArrayList<>();

	    try (Connection conn = ConnectDB.getConection();
	         PreparedStatement statement = conn.prepareStatement("SELECT * FROM SanPham WHERE maLoaiSP IN (SELECT maLoai FROM LoaiSP WHERE tenLoai = ?)")) {
	        ...
	        try (ResultSet resultSet = statement.executeQuery()) {
	            while (resultSet.next()) {
	                ...
	                boolean isNam = resultSet.getBoolean("isNam");
	                boolean conKinhDoanh = resultSet.getBoolean("ConKinhDoanh");
	                String maLoaiSP = resultSet.getString("maLoaiSP");
	                String hinhAnh = resultSet.getString("hinhAnh");
	                String maNCC = resultSet.getString("MaNCC");
	                float thue = resultSet.getFloat("thue");

	                NhaCC ncc = NhaCCDAO.GetNhaCC(maNCC);
	                LoaiSP lsp = SanPhamDAO.GetLoaiSP(maLoaiSP);

	                danhSachSanPham.add(new SanPham(maSP, tenSP, giaNhap, slTonKho, kichThuoc, mauSac, conKinhDoanh, isNam, ncc, hinhAnh, thue, lsp));
	            }
	        }
    ...
    public static String getTenLoaiByMaLoai(String maLoai) {
        String tenLoai = null;

        try (Connection con = ConnectDB.getConection();
             PreparedStatement statement = con.prepareStatement("SELECT tenLoai FROM LoaiSP WHERE maLoai = ?")) {
 ...
 // Lấy mã KM của sản phẩm   
    public static String layMaKhuyenMaiChoSanPham(String maSP) {
        String maKM = null;
        LocalDate ngayHienTai = LocalDate.now();
        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd");

        String sql = "SELECT maKM FROM KhuyenMai WHERE maSP = ? AND ? BETWEEN ngayBatDau AND ngayKetThuc";

        try (Connection connection = ConnectDB.getConection();
             PreparedStatement preparedStatement = connection.prepareStatement(sql)) {
            preparedStatement.setString(1, maSP);
            preparedStatement.setString(2, ngayHienTai.format(formatter));

            try (ResultSet resultSet = preparedStatement.executeQuery()) {
                if (resultSet.next()) {
                    maKM = resultSet.getString("maKM");
                }
            }
        } catch (SQLException e) {
            e.printStackTrace();
