...
public class PnlTKTK extends JPanel implements ActionListener, PropertyChangeListener, KeyListener {
	...
	public PnlTKTK() {
		...
		pnlTable.setBorder(new TitledBorder(new LineBorder(new Color(64, 64, 64)), "Chi ti\u1EBFt t\u1ED3n kho",
...
//		han che ngay cua lich
		...
		if (e.getSource() == btnXuatFile) {
			try {
				...
				if (userSelection == JFileChooser.APPROVE_OPTION) {
					...
					ToPDFController.xuatTKTK(fileToSave.getAbsolutePath(), ldDauKy, ldCuoiKy,
							(NguoiQuanLy) MainFrame.nv, tblCTTK,
							new int[] { Integer.parseInt(lblValTongSP.getText()),
									Integer.parseInt(lblValDaBan.getText()), Integer.parseInt(lblValNhapMoi.getText()),
	...
	public void propertyChange(PropertyChangeEvent evt) {
		...
		if (obj == dateDauKy.getDateEditor()
				&& !dateDauKy.getDate().toInstant().atZone(ZoneId.systemDefault()).toLocalDate().equals(ldDauKy)) {
			SetDate();
			dateCuoiKy.setMinSelectableDate(dateDauKy.getDate());
			LoadTable();

		} else if (obj == dateCuoiKy.getDateEditor() && !dateCuoiKy.getDate().toInstant().atZone(ZoneId.systemDefault())
	...
	private void SetDate() {
		if (dateDauKy.getDate() == null) {
			...
			ldDauKy = dateDauKy.getDate().toInstant().atZone(ZoneId.systemDefault()).toLocalDate();

		}
		if (dateCuoiKy.getDate() == null) {
			ldCuoiKy = LocalDate.now().minusYears(10);
		} else
			ldCuoiKy = dateCuoiKy.getDate().toInstant().atZone(ZoneId.systemDefault()).toLocalDate();
		ldCuoiKy = ldCuoiKy.plusDays(1);
		if (ldDauKy.isAfter(ldCuoiKy)) {
			JOptionPane.showMessageDialog(this, "Ngày đầu kỳ phải bé hơn ngày cuối kỳ");
		}
	}
...
//		Sắp xếp mã sản phẩm tăng dần
		...
		for (SanPham x : dsSP) {
			...
			tblCTTK.addRow(String.format("%03d", temp), x.getMaSP(), x.getTenSP(), x.getLoaiSP().getTenLoai(),
					x.getKichThuoc(), x.getMauSac(), slBan + x.getSlTonKho(), slBan, slTang, x.getSlTonKho());
			temp++;
